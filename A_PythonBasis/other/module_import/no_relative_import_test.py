...
# 相对路径导入
from rect_area import Rect_Area  # reac_area.py与test0.py位于同目录结构下)
...
# 相对路径导入
# 在与rect_volumes.py平级的的文件中执行rect_volumns.py,要求rect_volumns.py不能包含相对路径(. or .. or ...)
...
# 相对路径导入
from dir_example.print_hello import print_hello  # print_hello.py目录结构比test.py低

print_hello("python")  # 不包含相对路径(. or .. or ...)导入的代码
